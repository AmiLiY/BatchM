...
urlpatterns = [
    ...
    url(r'status/system/', asset_views.system_status),  # 汇报minion的系统状态信息
    url(r'host_status/$', asset_views.host_status, name='host_status'),
    url(r'host_status/(\d+)$', asset_views.host_status_detail),  # 获取单台服务器状态信息
    url(r'get_status_data$', asset_views.get_status_data, name='get_status_data'),  # 获取详细的状态信息
    url(r'saltstack_report$', asset_views.saltstack_report, name="saltstack_report"),  #
    url(r'GetServerHostStatus$', asset_views.server_host_status, name="get_server_host_status"),  # 获取运行次系统的服务状态信息
    url(r'put_cmd/(\d+)$', asset_views.run_shell, name="put_cmd"),  # 针对 单台minion执行命令
    url(r'groupshell/(\d+)$', asset_views.groupshell, name='groupshell'),  # 针对saltstack组做命令操作
    url(r'saltstack_group/(\d+)/$', asset_views.groupshell),  # 针对saltstack组做命令操作
