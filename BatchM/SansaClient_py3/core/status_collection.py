...
try:
    ...
    result = subprocess.getstatusoutput("pip install psutil")
    if result[0] == 0:
        import psutil
    else:
...
def system_load():
    ...
    :return:
    '''
    system_info_list=subprocess.getoutput("uptime").split(',')
    ...
    __status_info['hostname'] = subprocess.getoutput('hostname')
    ...
    __status_info['disk_max_usage'] = float(subprocess.getoutput(exec_command))
    cur_time=datetime.datetime.now()
    __status_info['update_time'] = cur_time.strftime('%Y-%m-%d %H:%M:%S.%u')
    __status_info['zombie_process'] = int(subprocess.getoutput("ps -ef |grep defunct |grep -v defunct|wc -l"))
