...
# import commands  # here haven't module commands in python3
import re
...
def collect():
    ...
    for key in filter_keys: #get basic system info
        try:
...
def nicinfo():
    ...
    raw_data= subprocess.check_output("ifconfig -a",shell=True).decode()
...
def raminfo():
    ...
    raw_data = subprocess.check_output("dmidecode -t 17",shell=True).decode()
    ...
    raw_total_size = subprocess.check_output(" cat /proc/meminfo|grep MemTotal ",shell=True).decode().split(":")
...
def osinfo():
    ...
    distributor = subprocess.check_output(" yum -y install redhat-lsb-core &&  lsb_release -a|grep 'Distributor ID'",shell=True).decode().split(":")
    #distributor = commands.getoutput(" lsb_release -a|grep 'Distributor ID'").split(":")
    release  = subprocess.check_output(" lsb_release -a|grep Description",shell=True).decode().split(":")
    #release  = commands.getoutput(" lsb_release -a|grep Description").split(":")
    minion_id = subprocess.check_output("grep ^id /etc/salt/minion |awk '{print $2}'",shell=True).decode()
    #minion_id = commands.getoutput("grep ^id /etc/salt/minion |awk '{print $2}'")
    data_dic ={
        "os_distribution": distributor[1].strip() if len(distributor)>1 else None,
        "os_release":release[1].strip() if len(release)>1 else None,
        "os_type": "linux",
        "salt_minion_id": minion_id,
    }
    return data_dic
def cpuinfo():
    ...
    cpu_model = raw_data["cpu_model"].decode().split(":")
    if len(cpu_model) >1:
    ...
    def linux(self):
        ...
        try:
            ...
            if 'Exit Code: 0x00' in  output[1]:
                '''
                if not have raid in the machine ,then the script will collect this path of /dev/sd* ,
                it will use sd* as disk info to report CMDB server
                '''
                capacity = subprocess.check_output("lsblk |grep disk |awk '{print $1,$4}'",shell= True).decode()[1]
                #capacity = commands.getstatusoutput("lsblk |grep disk |awk '{print $1,$4}'")[1]
                disk_info = []
